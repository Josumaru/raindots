...
Item {
    ...
    Process {
        id: configReader
        command: ["bash", "-c", "jq -r '.background.wallpaperPath // \"\"' ${XDG_CONFIG_HOME:-$HOME/.config}/illogical-impulse/config.json"]
        stdout: StdioCollector {
            onStreamFinished: {
                var path = text.trim();
                window.currentWallpaperPath = path;
                window.currentWallpaperName = path ? path.split("/").pop() : "";
                if (path && !window.originalWallpaperPath) {
                    window.originalWallpaperPath = path;
                }
    ...
    function revertIfUnapplied() {
        if (!window.isApplying && window.originalWallpaperPath) {
            Quickshell.execDetached(["awww", "img", "--transition-type", "center",
                "--transition-duration", "0.2", "--transition-fps", "60", window.originalWallpaperPath]);
        }
    }
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        if (isVideo) {
            Quickshell.execDetached(["bash", "-c",
                `pkill -f mpvpaper; for m in $(hyprctl monitors -j | jq -r '.[].name'); do mpvpaper -o 'no-audio loop hwdec=auto' "$m" "${originalFile}" & done`
            ]);
        } else {
            Quickshell.execDetached(["awww", "img", "--transition-type", "center", "--transition-duration", "0.6", "--transition-fps", "60", originalFile]);
        }

        // Save to config for next picker launch
        Quickshell.execDetached(["bash", "-c",
            `CONFIG="\${XDG_CONFIG_HOME:-\$HOME/.config}/illogical-impulse/config.json"; ` +
            `mkdir -p "$(dirname "$CONFIG")"; ` +
            `if [ -f "$CONFIG" ]; then ` +
            `  jq --arg p "${originalFile}" '.background.wallpaperPath = $p' "$CONFIG" > "$CONFIG.tmp" && mv "$CONFIG.tmp" "$CONFIG"; ` +
            `else ` +
            `  echo "{\\"background\\": {\\"wallpaperPath\\": \\"${originalFile}\\"}}" > "$CONFIG"; ` +
            `fi`
        ]);

        // Theme via IPC if shell is running (try both ii and rain configs)
        Quickshell.execDetached(["qs", "-c", "ii", "ipc", "call", "wallpapers", "apply", originalFile]);
        Quickshell.execDetached(["qs", "-c", "rain", "ipc", "call", "wallpapers", "apply", originalFile]);
        window.requestClose();
    }
   ...
   onVisibleChanged: {
    ...
    function isVideoName(name) {
        return /\.(mp4|mkv|mov|webm|avi)$/i.test(String(name));
    }
    ...
    function tryFocus() {
        if (initialFocusSet && !window.currentWallpaperName) return;

        if (localProxyModel.count > 0) {
            let foundIndex = -1;

            if (window.currentWallpaperName) {
                for (let i = 0; i < localProxyModel.count; i++) {
                    let fname = localProxyModel.get(i).fileName || "";
                    let clean = window.getCleanName(fname);
                    if (fname === window.currentWallpaperName || clean === window.currentWallpaperName) {
                        foundIndex = i;
                        break;
                    }
                }
    ...
    FolderListModel {
        ...
        onCountChanged: {
            ...
            Qt.callLater(() => window.syncLocalModel());
        }
        onStatusChanged: {
            if (status === FolderListModel.Ready) Qt.callLater(() => window.syncLocalModel());
        }
    }
        ...
        const extractScript = `
            ...
            for file in "$THUMBS"/*; do
                if [ -f "$file" ]; then
                    ...
                    if [ $found -eq 0 ]; then
                        extLower=$(echo "$filename" | sed 's/.*\.//' | tr '[:upper:]' '[:lower:]')
                        case "$extLower" in
    ...
    function doPreviewWallpaper(safeFileName) {
        ...
        window.targetWallName = safeFileName;
        Quickshell.execDetached(["awww", "img", "--transition-type", "center",
            "--transition-duration", "0.2", "--transition-fps", "60", originalFile]);
    }
    ...
    Shortcut {
        ...
        onActivated: {
            ...
            if (view.currentIndex >= 0 && view.currentIndex < targetModel.count) {
                ...
                if (fname) window.previewWallpaper(String(fname));
            }
        }
    }
    Shortcut {
    ...
    Shortcut { sequence: "Escape"; enabled: !window.isApplying; onActivated: { if (window.currentFilter === "Search") { window.currentFilter = "All"; } else { window.revertIfUnapplied(); Qt.quit(); } } }
    ...
    function syncLocalModel() {
        ...
        if (srcModel.status === FolderListModel.Ready && srcModel.count > 0) {
            for (let i = 0; i < srcModel.count; i++) {
                let fn = srcModel.get(i, "fileName");
                let fu = srcModel.get(i, "fileUrl");
                if (fn !== undefined && !seenNames.has(fn)) {
                    localProxyModel.append({ "fileName": fn, "fileUrl": String(fu) });
                    seenNames.add(fn);
                }
            }
        ...
        if (!window.initialFocusSet && localProxyModel.count > 0) {
            if (window.currentWallpaperName) {
                for (let i = 0; i < localProxyModel.count; i++) {
                    let fname = localProxyModel.get(i).fileName || "";
                    if (fname === window.currentWallpaperName || window.getCleanName(fname) === window.currentWallpaperName) {
...
delegate: Item {
            ...
            readonly property string thumbUrl: {
                if (!window.isVideoName(safeFileName)) return fileUrl;
                let nameNoExt = safeFileName.replace(/\.[^/.]+$/, "");
                return window.thumbDir + "/" + nameNoExt + ".jpg";
            }
            ...
            readonly property bool isVideo: /\.(mp4|mkv|mov|webm|avi)$/i.test(safeFileName)
