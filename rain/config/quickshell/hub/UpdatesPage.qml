...
// Updates section, wired to `rain status --json`.
Item {
    id: page
    ...
    function setInterval(k) {
        if (page.interval === k) return;
        page.interval = k;
        saveInterval.command = [Directories.rainBin, "hub", "config", "set", "update_interval", k];
        saveInterval.running = true;
    }

    Process {
        id: loadInterval
        command: [Directories.rainBin, "hub", "config", "get", "update_interval"]
        running: true
    ...
    function startUpdate() {
        Quickshell.execDetached(["kitty", "-e", "sh", "-c", "RYOKU_UPDATE_UI=hub exec rain update"]);
    }
    ...
    Item {
        ...
        Text {
            ...
            text: page.phase === "running"
                ? "Update running"
                : (Updates.branch + "  ·  " + Updates.currentVersion)
            color: Theme.faint
            font.family: Theme.mono
            font.pixelSize: 12
        }
